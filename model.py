...
def green_block(inp, filters, data_format='channels_first', name=None):
    ...
    inp_res = Conv3D(
        ...
        name='Res_%s'%name if name else None)(inp)
    ...
    x = GroupNormalization(
        ...
        name='GroupNorm_1_%s'%name if name else None)(inp)
    x = Activation('relu', name='Relu_1_%s'%name if name else None)(x)
    x = Conv3D(
        filters=filters,
        kernel_size=(3, 3, 3),
        strides=1,
        padding='same',
        data_format=data_format,
        name='Conv3D_1_%s'%name if name else None)(x)

    x = GroupNormalization(
        groups=8,
        axis=1 if data_format == 'channels_first' else 0,
        name='GroupNorm_2_%s'%name if name else None)(x)
    x = Activation('relu', name='Relu_2_%s'%name if name else None)(x)
    x = Conv3D(
        filters=filters,
        kernel_size=(3, 3, 3),
        strides=1,
        padding='same',
        data_format=data_format,
        name='Conv3D_2_%s'%name if name else None)(x)

    out = Add(name='Out_%s'%name if name else None)([x, inp_res])
    return out
